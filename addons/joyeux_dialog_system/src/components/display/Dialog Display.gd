...
@export var dialog = "" # (String, FILE, "*.mtalk ; Dialog files")
	...
	for connection in conf.get_value("config", "connections"):
		if connection.get("from") == "Start":
			...
			if temp is GDScriptFunctionState:
				next_node = await temp.completed
			elif temp is String:
				next_node = temp
...
func evaluate_node(node : String) -> String:
	...
	if matches.has(node):
		var answer = await ChoiceMenu.get_text_answer().completed
	...
	if choices.has(node):
		var answer = await ChoiceMenu.get_multi_answer(choices.get(node)).completed
...
func _on_Auto_pressed():
	if PauseButton.pressed:
		return
	await get_tree().create_timer(10*Text.text.length()).timeout
